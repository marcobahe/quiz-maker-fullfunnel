 ...
 * Verifies whether a paywall purchase was paid.
 * DB is source of truth; Stripe is fallback for bootstrap.
...
export async function GET(request, { params }) {
  ...
  try {
    ...
    if (!parsedQuery.success) {
      return NextResponse.json(
        { error: 'Dados inválidos', details: parsedQuery.error.flatten().fieldErrors },
        { status: 400 }
      );
    }
    ...
    if (!quiz.paywallEnabled) {
      return NextResponse.json(
        { error: 'Paywall não está ativo para este quiz.' },
        { status: 400 }
      );
    }

    // ── DB is source of truth ─────────────────────────────────
    const purchase = await prisma.quizPurchase.findUnique({
      where: { stripeSessionId: sessionId },
    });

    if (purchase) {
      // Security: ensure the purchase belongs to this quiz
      if (purchase.quizId !== quiz.id) {
        return NextResponse.json(
          { error: 'Sessão inválida para este quiz.' },
          { status: 400 }
        );
      }
      ...
      return NextResponse.json({
        ...
        session: {
          id: sessionId,
          status: paid ? 'complete' : purchase.status,
    ...
    // ── Fallback: Stripe API (bootstrap / webhook delay) ──────
