...
export async function POST(request, { params }) {
  ...
  try {
    ...
    if (!quiz.paywallEnabled) {
      return NextResponse.json({ error: 'Paywall não está ativo para este quiz.' }, { status: 400 });
    }

    if (!quiz.paywallStripePriceId) {
      return NextResponse.json(
        { error: 'Preço do paywall não configurado.' },
        { status: 503 }
      );
    }
    ...
    const mode = quiz.paywallType === 'subscription' ? 'subscription' : 'payment';
    ...
    const checkoutSession = await stripe.checkout.sessions.create({
      ...
      // For one-time payments, allow customer email collection
      ...(mode === 'payment'
        ? {
